...
if "board" not in st.session_state:
    ...
    st.session_state.selected = set()  # (r, c) 튜플 저장
...
with col1:
    if st.button("✅ 제거하기 (합이 10일 때)", use_container_width=True, type="primary"):
        if selected_sum == 10 and len(st.session_state.selected) > 0:
            ...
            st.session_state.score += len(st.session_state.selected)
            st.session_state.selected = set()
            st.success(f"+{len(st.session_state.selected)}점!")
            st.rerun()
        else:
    ...
    for c in range(COLS):
        ...
        if value == 0:
            ...
            if cols[c].button(label, key=key):
                if is_selected:
                    st.session_state.selected.remove((r, c))
                else:
